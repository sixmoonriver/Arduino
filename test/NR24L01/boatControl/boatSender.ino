...
uint8_t convert[5]={0};
uint8_t cam_value= 0;
uint16_t xSourceValue,ySourceValue,camSourceValue,temp_value=0;
uint8_t x_value,y_value,last_xvalue,last_yvalue,con_value,last_convalue=0;
uint32_t trans_value = 0;
int xpin = 0;  // 方向输入脚
int ypin = 1;  // 油门输入脚
//开关引脚定义
int switchPin1 = 6;
...
void loop()
{
  ...
  //value = random(255); 
  //读取摄像头角度控制值
  camSourceValue = analogRead(camControl);
  cam_value = map(camSourceValue,0,1023,0,15);
  ...
  if(x_value != last_xvalue | y_value != last_yvalue | con_value!= last_convalue)
  {
    //发送的数据组装
	trans_value = 0;
	if(con_value > 0){
	    trans_value = con_value + trans_value;
		trans_value = trans_value<<16; //控制数据左移16位，到16~24位；	
	}
	if(x_value > 0){
		temp_value = 0;
